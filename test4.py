# command to run this
# python test4.py --db all_data.npy --labels all_labels.npy --bs 10 --layers 3 --cells 20 --iters 10000 --fracTest 0.2 --nframes 0 --save ./0
...
parser.add_argument("--fracTest", help="fraction of test samples",type=float, default=0.2) ;
...
alldata = np.zeros([alldataa.shape[0], alldataa.shape[2]])
rf = alldataa[:,0,:]
print "rf", rf.shape

alldata = np.zeros([alldataa.shape[0], alldataa.shape[1]+f2app, alldataa.shape[2]])

for i1 in range(alldataa.shape[1]):
    alldata[:,i1,:] = rf

for i in range(alldataa.shape[1]):
    alldata[:,i+f2app,:] = alldataa[:,i,:]
...
n_steps = alldataa.shape[1]+f2app   # timesteps
print "n_steps", n_steps
outputFrame = n_steps-1
print "outputFrame", outputFrame;
#outputFrame = args.output ;
...
pred = RNN(x, weights, biases, n_steps, n_layers, n_hidden, outputFrame)
