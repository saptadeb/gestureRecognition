# to prepend (add before) nframes (N number of frames) of RANDOM to the data

# command to run this
# python rnn_lstm_train_rnfrmapp.py --db all_data.npy --labels all_labels.npy --bs 10 --layers 3 --cells 20 --iters 10000 --output 39 --fracTest 0.2 --load ./model.ckpt --nframes 1 --save ./1.ckpt
...
ri = random.randint(0,alldataa.shape[0]) ##random integer for sample
rs = alldataa[ri,:,:]
print "random smpl", rs.shape

rf = np.append(rs,rs[:20,:], axis=0)
print "rf", rf.shape

alldata = np.zeros([alldataa.shape[0], alldataa.shape[1]+20, alldataa.shape[2]])

for i1 in range(alldata.shape[0]):
     alldata[i1,:,:] = rf
